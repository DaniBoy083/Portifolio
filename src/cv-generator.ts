...
async function fetchGitHubSummary(githubUrl: string): Promise<GitHubSummary | null> {
...
function buildProjectsSection(projects: ProjectSummary[], mode: CurriculumMode): string {
    if (!projects.length) {
...
function buildGitHubSection(summary: GitHubSummary | null, fallbackUrl: string, mode: CurriculumMode): string {
    if (!summary) {
        if (!fallbackUrl) {
...
function buildCurriculumMarkdown(snapshot: PortfolioSnapshot, githubSummary: GitHubSummary | null, mode: CurriculumMode): string {
    ...
    lines.push(
        '## Projetos',
        buildProjectsSection(snapshot.projects, mode),
        '',
        '## Certificacoes',
        toBulletList(snapshot.certifications),
        ''
    );
    ...
    lines.push(
        '## Dados GitHub (publicos)',
        buildGitHubSection(githubSummary, snapshot.githubUrl, mode),
        '',
        '## Observacao sobre LinkedIn',
        '- O LinkedIn e referenciado pelo link publico no portfolio.',
        '- Para incluir detalhes adicionais (experiencias e formacoes detalhadas), complemente manualmente no documento final.'
    );
...
function downloadFile(content: string, name: string, mimeType: string): void {
